...
valley-polarised transport studies
\cite{Yesilyurt2026valleyfilter,Yesilyurt2026phasegate} and is being
released for general use in device-level electron-optics modelling.
A typical user defines a YAML configuration describing the grid,
dispersion, injection, potential, and boundaries; invokes
\texttt{run\_simulation()} either directly or through one of the
parallel sweep drivers; and analyses the resulting transport
observables and snapshot series.
...
used it as the computational engine for studies of valley-resolved
transport in tilted Dirac channels
\cite{Yesilyurt2026valleyfilter} and for an analysis of all-electrostatic
valley phase gates \cite{Yesilyurt2026phasegate}; both manuscripts
cite the package as their numerical tool. The package enables
systematic parameter scans (barrier geometry, tilt magnitude and
direction, intervalley coupling amplitude, source--drain bias) that
are impractical with tight-binding scattering-matrix codes on
...
The author thanks A.\,D. Zabolotskii (Institute for Spectroscopy RAS)
for correspondence on the 8-Pmmn borophene tight-binding model and
for clarifying which borophene phases have been experimentally
synthesised, and the Nanoelectronics Research Center for computing
resources.
